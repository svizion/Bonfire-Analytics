...
<div class="dashboard_box dashboard_focus">
	...
	<p style=""><span style="font-size:18px; font-weight:bold"><?= array_sum(array_values($visitors)); ?></span> Visits</p>
	<p style=""><span style="font-size:18px; font-weight:bold"><?= array_sum(array_values($pageviews)); ?></span> Page views</p>
	<p style=""><span style="font-size:18px; font-weight:bold"><?= round(array_average(array_values($visitsperhour)),2); ?></span> Pages/hour</p>
</div>
...
// <![CDATA[
	var so = new SWFObject("<?=base_url()?>static/swf/backend/amcharts/amline.swf", "linechart", "100%", "250", "8", "#FFFFFF");
	//so.addVariable("data_file", encodeURIComponent("<?=base_url()?>../static/swf/backend/amcharts/amline_data.xml"));
	so.addVariable("settings_file", encodeURIComponent("<?=base_url()?>static/swf/backend/amcharts/amline_settings.xml"));
	so.addVariable("chart_data", encodeURIComponent('<?=$chart_data?>'));
	so.addVariable("path", '<?=base_url()?>');
	so.write("linechart");
// ]]>
</script>